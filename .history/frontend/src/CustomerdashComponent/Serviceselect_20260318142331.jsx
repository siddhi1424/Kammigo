  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-10">
      {services.map((item, index) => (
        <div
          key={index}
          onClick={() => navigate(`/service/${item.name.toLowerCase()}`)}
          className="border rounded-2xl cursor-pointer overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-1"
        >
